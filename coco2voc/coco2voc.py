...
def coco2voc(anns_file, target_folder, n=None, compress=True):
    ...
    for i, img in enumerate(coco_imgs):
        ...
        Image.fromarray(class_seg).convert("L").save(class_target_path + '/' + str(file_name))
        Image.fromarray(instance_seg).convert("L").save(instance_target_path + '/' + str(file_name))
        ...
        image_name_list.write(str(file_name)+'\n')
